...
for acc in accs:
	...
	data_dict['id'] = acc
	data_dict['number_of_properties'] = np.unique(acc_props['number_of_properties'])[0]
...
print np.amax(accs_df['number_of_properties'].values)
ax.scatter(values, accs_df['number_of_properties'].values, color=color)
plt.show()