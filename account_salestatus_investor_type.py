...
for acc in accs:
	...
	data_dict['id'] = acc
	data_dict['investor_type'] = np.unique(acc_props['investor_type'])[0]
...
print np.amax(accs_df['investor_type'].values)
ax.scatter(values, accs_df['investor_type'].values, color=color)
plt.show()